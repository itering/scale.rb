module Scale
  module TypeBuilder
    class << self
      ...
      def get(type_def)
        if type_def.class == ::String 
          if type_def.start_with?('Scale::Types::')
            return get_type_const(type_def)
          end

          # find the final type from registry
          type_def = fix_name(type_def)
          type_def = get_final_type_from_registry(type_def)
        end
      ...
      def build(type_def)
        ...
        # 5. struct, 6. enum, 7. set
          ...
          type_def.transform_keys!(&:to_sym)
          if type_def[:type] == "struct"
            ...
            raise Scale::TypeBuildError.new("Not support: #{type_info}")
          end

        end
      end

      def get_type_const(type_name)
        Object.const_get(type_name)
      rescue => e
        raise Scale::TypeBuildError.new("There is no type constantized with name `#{type_name}`")
      end

      def build_vec(type_def)
        inner_type_str = type_def.scan(/\AVec<(.+)>\z/).first.first
      ...
      def build_option(type_def)
        inner_type_str = type_def.scan(/\AOption<(.+)>\z/).first.first
      ...
      def build_array(type_def)
        scan_result = type_def.scan(/\[(.+);\s*(\d+)\]/)
      ...
      def build_tuple(type_def)
        scan_result = type_def.scan(/\A\((.+)\)\z/)
